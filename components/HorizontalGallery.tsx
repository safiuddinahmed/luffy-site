...
export default function HorizontalGallery() {
  ...
  // Much wider range to show all 15 images
  const x = useTransform(scrollYProgress, [0, 1], ["0%", "-85%"]);

  // Larger vertical offsets for dramatic staggering (±250px range)
  const verticalOffsets = [
    0, -150, 120, -80, 180, -120, 100, -180, 140, -100, 200, -140, 80, -200, 160
  ];

  return (
    <div ref={containerRef} className="relative bg-white h-[400vh]">
      ...
      <div className="sticky top-0 h-screen overflow-hidden">
        ...
        <div className="absolute top-16 left-0 right-0 z-10 max-w-7xl mx-auto px-8">
          ...
          >
            <p className="text-xs uppercase tracking-[0.3em] text-gray-500 mb-4 font-medium">
              ICONIC MOMENTS
            </p>
            <h2 className="text-5xl md:text-6xl lg:text-7xl font-semibold tracking-tight text-black leading-tight">
        ...
        {/* Horizontal Scrolling Gallery with Staggered Heights */}
        <div className="absolute inset-0 flex items-center">
          ...
          >
            {iconicMoments.map((moment, index) => (
              ...
              >
                {/* Card Container */}
                <div className="relative w-[380px] h-[480px] bg-white overflow-hidden shadow-sm hover:shadow-xl transition-shadow duration-500">
                  {/* Image */}
            ...
            {/* Spacer to ensure smooth scroll */}
            <div className="w-8 flex-shrink-0" />
          </motion.div>
        </div>

        {/* Scroll Progress Indicator */}
        <div className="absolute bottom-16 left-0 right-0 z-10 max-w-7xl mx-auto px-8">
          <motion.div
            initial={{ opacity: 0 }}
